...
##########################################################################################
class negative_log_marginal_lik_MRI_multidats(torch.nn.Module):
    def __init__(self, prior_kernel, base_kernel, batch_size,  Xs_tensor, Ys_tensor, scores_Tensor, flag_if_use_medianheuristic):
        ...
        """
        super(negative_log_marginal_lik_MRI_multidats, self).__init__()
    ...
    def forward(self, batch_sample_indices, beta_cstkernel = 1):
        ...
        for idx in range(self.T):
            ...
            lam = 1e-6
            bad_cond = 1. / np.linalg.cond(k_Xlb_Xlb.detach().numpy()) < 10 ** (-6)  # machine precision
...
class TuneKernelParams_mllk_MRI_multidats(object):
    ...
    def do_optimize_logmll(self, batch_size, flag_if_use_medianheuristic, beta_cstkernel=1, lr=0.01, epochs=100, verbose=True):
        neg_mll = negative_log_marginal_lik_MRI_multidats(self.prior_kernel, self.base_kernel, batch_size,  self.Xs_tensor, self.Ys_tensor, self.scores_Tensor, flag_if_use_medianheuristic)
        optimizer = torch.optim.Adam(neg_mll.parameters(), lr=lr)
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.99)
...
class negative_log_marginal_lik_MRI_multidats_unbalanced(torch.nn.Module):
    def __init__(self, prior_kernel, base_kernel, batch_size, Xs_tuple, Ys_tuple, scores_tuple, flag_if_use_medianheuristic):
        ...
        :param flag_if_use_medianheuristic: whether or not to use median heuristic
        """
        super(negative_log_marginal_lik_MRI_multidats_unbalanced, self).__init__()
        ...
        if flag_if_use_medianheuristic == True:
            ...
            self.base_kernel_parm2_raw = torch.log(torch.ones(1, dtype=torch.float32, requires_grad=False) *  torch.median(torch.cdist(_Xs_flattened, _Xs_flattened, p=2) ** 2) / torch.log(Xs_flattened.size()[0] * torch.ones(1))  )# Use median heuristic for the lengthscale
    ...
    def forward(self, batch_sample_indices_dict, beta_cstkernel = 1):
        ...
        for idx in range(self.T):
            ...
            if Y_lb.dim() == 1:
                Y_lb = Y_lb.unsqueeze(dim=1)  #: ensure Y is a column vector
...
class TuneKernelParams_mllk_MRI_multidats_unbalanced(object):
    # : MRI_Case1  is: when we have one training set X, with a vector-valued function, and Pi is identical to all componenet of the vector-valued funcion
    def __init__(self, prior_kernel, base_kernel, Xs_tuple, Ys_tuple, scores_tuple):    #prior_kernel, base_kernel, X_train, Y_train, score_tensor
        """
    ...
    def do_optimize_logmll(self, batch_size, flag_if_use_medianheuristic, beta_cstkernel=1, lr=0.01, epochs=100, verbose=True):
        neg_mll = negative_log_marginal_lik_MRI_multidats_unbalanced(self.prior_kernel, self.base_kernel,batch_size, self.Xs_tuple, self.Ys_tuple, self.scores_tuple, flag_if_use_medianheuristic)
        optimizer = torch.optim.Adam(neg_mll.parameters(), lr=lr)
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.99)
