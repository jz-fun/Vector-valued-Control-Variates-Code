...
##=======================================================================================================================================
## 1. For Scalar-Valued functions
##=======================================================================================================================================

class penalized_ls_objective_scalarvaluedfunc(torch.nn.Module):
    def __init__(self, optim_base_kernel_parms, prior_kernel, base_kernel, batch_size, X_whole_tr, Y_whole_tr, score_tensor_X_whole_tr):
        ...
        """
        super(penalized_ls_objective_scalarvaluedfunc, self).__init__()
    ...
    def forward(self, batch_sample_indices, regularizer_const):
        ...
        y_offset_c = Y_batch - self.c   #  pytorch will broadcast this to every element of self.Y
        if y_offset_c.dim() == 1:
            y_offset_c.unsqueeze(dim=1)  # set to be a column vector

        Y_batch_preds = k_Xb_X @ self.theta + self.c
        scalable_objective = (Y_batch.squeeze() - Y_batch_preds.squeeze()).pow(2).sum()/self.batch_size + regularizer_const * (self.theta.t() @ self.theta)      # Zhuo: for the results on the current draft
...
class SV_CV_scalarvaluedfuncs_model(object):
    ...
    def do_optimize_sv_CV(self, regularizer_const, batch_size = 10 , lr=0.2, epochs=200, verbose=True):
        ...
        for i in range(epochs):
            ...
            saved_BQ_est[i] = fitting_obj.c   # the name stems from the relationship between BQ and CF; could use other names. and use 'c' as a biased estimate being sometimes preferrable as it has lower MSE
            saved_loss[i] = out
    ...
    def do_integration(self, X_train, Y_train):
        ...
        # This is the last item in save_BQ_est
    ...
    def do_integration_ControlVariates(self, X_test, Y_test, score_X_test_tensor):
        ...
        kernel_obj.base_kernel_parm1 = self.fitting_obj.base_kernel_parm1
        kernel_obj.base_kernel_parm2 = self.fitting_obj.base_kernel_parm2
